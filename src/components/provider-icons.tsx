...
/** OpenAI mark from stablyai/orca — inherits currentColor. */
...
/** OpenCode Go mark adapted from stablyai/orca for theme blending. */
export function OpenCodeGoIcon({ size = 16, className }: IconProps) {
  const height = Math.round((size * 300) / 240);
  return (
    <svg
      width={size}
      height={height}
    ...
    >
      <path
        fill="currentColor"
        fillRule="evenodd"
        d="M0 0h240v300H0V0Zm60 60h120v180H60V60Z"
      />
    </svg>
  );
}
...
/** Official Composio logomark from brand.composio.dev — monochrome for theme blending. */
export function ComposioIcon({ size = 16, className }: IconProps) {
...
/**
 * Official Cursor cube mark from cursor.com/brand (2D logomark used in
 * Cursor's own UI packages / marketplace avatars) — monochrome for theme blending.
 */
export function CursorIcon({ size = 16, className }: IconProps) {
  const height = Math.round((size * 56) / 49);
  return (
    <svg
      width={size}
      height={height}
...
/** Official Antigravity mark (lobe-icons / Antigravity IDE) — monochrome for theme blending. */
...
export function ProviderIcon({
  ...
  if (provider === "composio") {
    return <ComposioIcon size={size} className={className} />;
  }
